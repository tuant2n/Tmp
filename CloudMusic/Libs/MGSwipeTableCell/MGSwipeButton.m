...
{
    return [self buttonWithTitle:title icon:icon backgroundColor:color padding:20 callback:callback];
}
...
+ (instancetype)buttonWithTitle:(NSString *)title icon:(UIImage*)icon backgroundColor:(UIColor *)color insets:(UIEdgeInsets)insets callback:(MGSwipeButtonCallback)callback
{
    ...
    [button.titleLabel setFont:[UIFont fontWithName:@"HelveticaNeue-Medium" size:10.0]];
